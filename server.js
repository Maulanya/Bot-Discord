...
bot.on("message", async (message) => {
  ...
  // Periksa apakah pesan tersebut dari DM
  if (message.channel.type === 'dm') {
    const dmEmbed = new discord.MessageEmbed()
      .setAuthor(bot.user.username + ' UnboxLabs Community', bot.user.displayAvatarURL())
      .addField('<:message:1153315110228590603> Pesan Otomatis', `Hai <@${message.author.id}>, ada yang bisa dibantu?`)
      .addField('<:staff:1153327728913829889> Staff', 'Mungkin anda berusaha untuk menghubungi staff <:contact:1153327091820007546>')
      .addField('<:discord:1153327696085000242> Bergabung', '[Bergabung dengan Komunitas kami](https://www.discord.gg/unboxlabs)')
      .setColor('#03fc0f')
      .setTimestamp();
    ...
    const button = new disbut.MessageButton()
      .setLabel('Hubungi Staff')
      .setStyle('url')
      .setURL('https://discordapp.com/users/1072861937689436170');

    try {
      const user = await bot.users.fetch(message.author.id); // Ambil info pengguna DM
      await user.send({ embed: dmEmbed, component: button });
    } catch (error) {
      console.error('Gagal mengirim pesan DM ke pengguna:', error);
    }
  }
  try {
...
`)
    ...
    console.error('Terjadi kesalahan dalam pemanggilan database:', error);
  }
...
`, stat[user.presence.status])
  ...
  let testo = "."; // Default prefix jika tidak ada server (DM)

  if (message.guild) {
    testo = bot.db.get(`${message.guild.id}_prefix`) || ".";
  }
    ...
    oldRole.guild.fetchAuditLogs().then((logs) => {
      ...
      if (oldRole.permissions !== newRole.permissions) {
        let roleUpdate = new discord.MessageEmbed()
          ...
          .setColor("RANDOM")
          .setDescription(
            `**\n**:first_place: Successfully \`\`CHANGED\`\` **${oldRole.name
...
bot.on("guildCreate", (guild) => {
  ...
  bot.channels.cache
    .get("1112711876544057424")
    .send(
      `New Guild Joined: (**${guild.name}**)\nID: (**${guild.id}**)\nThis Guild has **${guild.memberCount}** Members`
    );
  guild.systemChannel
    .send(
      `**thanks for adding ITE BOT**\nPrefix \`${testo}\` and command help \`${testo}help\`, bot prefix you can change by using command \`${testo}setprefix\`.\n***Hello***\nHello, I'm a bot (ITE BOT), I'm a bot system, what I mean by my system is to make it easier for your server system to switch to bots, commands that are safe without any symptoms, if there are symptoms, please type prefix \`${testo}report\` to send bugs or something problematic on bots.\n***Support***\nSupport us by voting for our bot or joining our discord server, don't forget to share the ITE BOT invite link (<https://discord.com/oauth2/authorize?client_id=975706013124661252&scope=bot&permissions=1543564542>) with your friends.\n**Join our server**:\n<https://iktindonesia.ml/discord>`
    )
    .catch((x) => x.return);
});
...
res.send("Unboxlabs Bot by - Maulana")
})
